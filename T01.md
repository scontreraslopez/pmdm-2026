...
* [El argumentario a favor del multiplataforma](https://www.youtube.com/watch?v=yye7rSsiV6k) — un análisis con una preferencia clara por esta vía.
* [Philipp Lackner: por qué sigue apostando por Android nativo](https://www.youtube.com/watch?v=fPm4yGATPDM) — mantiene KMP en el radar, pero recomienda esperar a que gane adopción.
...
Casi todas las comparativas que encontraréis por ahí mezclan cosas distintas y acaban en una lista de ventajas e inconvenientes que no ayuda a decidir nada. Para ordenarlo basta con separar dos preguntas que son **independientes entre sí**:
...
* **Lenguaje: Kotlin.** Google mantiene una estrategia *Kotlin-first* y lo recomienda para toda aplicación nueva. Java sigue oficialmente soportado y es interoperable: no está muerto —hay muchísimo código Java en producción—, simplemente ya no es por donde se empieza un proyecto nuevo.
* **Interfaz: Jetpack Compose**, el toolkit moderno recomendado por Google, preparado para teléfonos, tablets, plegables y Wear OS. El sistema clásico de *Views* con XML sigue siendo compatible y lo encontraréis en montones de aplicaciones existentes.
* **IDE: Android Studio.**

En el **ecosistema Apple**:

* **Lenguaje: Swift.**
* **Interfaz: SwiftUI**, que permite reutilizar buena parte de la misma implementación entre iOS, iPadOS, macOS, watchOS y visionOS. UIKit y Objective-C siguen siendo muy relevantes en proyectos ya existentes.
* **IDE: Xcode** y, por tanto, un Mac.

Fijaos en que **las dos plataformas han hecho el mismo viaje a la vez**: de un sistema clásico (Views, UIKit) a uno **declarativo** (Compose, SwiftUI). Declarativo significa que describís *cómo debe verse la interfaz para un estado dado* y el framework se encarga de actualizarla cuando ese estado cambia, en lugar de ir manipulando la pantalla paso a paso. Es el cambio de mentalidad que más os va a costar en **T03** y **T04**; y que Apple llegara por su cuenta a la misma solución es la mejor prueba de que no es una moda pasajera de Google.
...
**React Native: interfaz compartida sobre componentes nativos.** Se programa en JavaScript o, lo habitual hoy, en TypeScript con React, y el framework traduce eso a componentes nativos de cada sistema. Encaja especialmente bien en equipos que ya dominan React y TypeScript. El propio proyecto recomienda hoy arrancar con **Expo**, que aporta la infraestructura y las herramientas para llegar a producción.

> [!NOTE]
> *(Para saber más.)* Muchos tutoriales todavía describen React Native mediante el antiguo *"puente de JavaScript"*. Esa descripción está obsoleta: las versiones modernas usan la llamada **New Architecture**, con comunicación directa entre JavaScript y código nativo y un *renderer* reescrito sobre un núcleo C++. Desde la versión 0.82, de octubre de 2025, es la única arquitectura disponible.

**Flutter: interfaz compartida con motor gráfico propio.** Usa el lenguaje **Dart** y trae su propio sistema de widgets y su propio motor de dibujo. A diferencia de una app híbrida, **no** renderiza dentro de un WebView: controla directamente el pintado de la interfaz. Su gran fortaleza es conseguir una UI idéntica y muy personalizada en todas las plataformas. Aquí también hay una descripción anticuada muy extendida: Flutter ya **no** dibuja con Skia, sino con **Impeller**, su motor por defecto desde Flutter 3.27.
...
Y aquí está la idea que de verdad importa: **Kotlin Multiplatform no es un punto, es un continuo.** Podéis compartir solo la lógica, o la lógica y algunas pantallas, o casi todo, y moveros por esa línea según lo que pida el proyecto:
...
Una **web *responsive*** es simplemente un sitio web cuya interfaz se adapta al tamaño y las características del móvil. Se ejecuta en el navegador y no se instala nada.

Una **PWA** (*Progressive Web App*) añade lo necesario para acercarse a la experiencia de una app instalada: con un *Web App Manifest* obtiene su propio icono y se abre en una ventana independiente del navegador. Muchas usan *service workers* para cachear contenido y funcionar sin conexión, aunque técnicamente eso ya no es un requisito para poder instalarlas.

Lo que sí cambia mucho es **qué permite cada navegador y cada sistema operativo**. En iOS, desde la versión 16.4, una PWA puede añadirse a la pantalla de inicio desde el menú Compartir de Safari; pero iOS sigue sin admitir `beforeinstallprompt`, que es lo que permite a una web ofrecer su propio botón de "instalar". Traducido: en Android podéis guiar al usuario, y en iPhone tenéis que explicarle dónde pulsar.

Su gran ventaja es la distribución: una PWA se reparte con una URL, se encuentra desde un buscador y no pasa por la revisión de ninguna tienda, con todo lo que eso ahorra según veíamos más arriba. Y si aun así se quiere estar en Google Play, en Android existe la **Trusted Web Activity (TWA)**, que abre el contenido de la PWA a pantalla completa dentro de una app Android.

> [!NOTE]
> *(Para saber más.)* Una TWA no es un WebView: el contenido lo sigue renderizando el navegador del dispositivo, mediante el mecanismo de las *Custom Tabs*. Es una distinción que importa para el rendimiento y para saber qué APIs web están disponibles.
...
Una advertencia sobre esta tabla, porque las comparativas de este tipo se leen mal con facilidad: la columna de multiplataforma **agrupa cosas muy distintas**. KMP con interfaz nativa da una sensación de app idéntica a la nativa, mientras que en una app Flutter se nota, para bien o para mal, que no usa los componentes del sistema. Usadla como orientación, no como sentencia.

### **Entonces, ¿cuál elijo?**

Reglas de bolsillo, que no sustituyen a pensar pero evitan la mayoría de los disgustos:

* **Una sola plataforma, o exigencia máxima de rendimiento e integración** (juegos, edición de vídeo, cámara, sensores, *wearables*): **nativo**.
...
¿Ha despegado, entonces? Todavía no tiene la cuota de Flutter ni de React Native, pero ha dejado de ser una promesa. Y lo que más os interesa a vosotros: **lo que vais a aprender en este módulo es la puerta de entrada natural a KMP**. Kotlin y Compose son exactamente la misma base, así que dar el salto consiste en compartir código que ya sabréis escribir, no en empezar de cero.
