...
La sentencia `for` en Kotlin es similar a la de Java en intención, pero **más expresiva y más cercana a iteradores** que a contadores clásicos.
...
Internamente, Kotlin traduce el `for` a:
...
Ojo, es similar a como ocurre en Java, no se pueden borrar elementos de la colección mientras iteras con el `for` *clásico*. En su lugar tienes que utilizar un iterador explícito como en Java.
...
while (it.hasNext()) {
    val n = it.next()
    if (n % 2 == 0) {
        it.remove()  // Si intentas borrar del for, salta ConcurrentModificationException
    }
}
```

Todo sea dicho, en Kotlin las colecciones suelen modificarse con funciones de orden superior como `filter`, `map`, `forEach`, etc., que son más idiomáticas y seguras.
