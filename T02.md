...
=======
En **Java**, estamos acostumbrados a la verbosidad (sí, este adjetivo existe en castellano). Además todo está empotrado en clases *wrapper*, lo que hace que incluso un simple "Hola Mundo" requiera varias líneas de código. *Nota*: Sé que esto mejora en java 25:
>>>>>>> 1e5d881fd1342932a155a404eb1d4076fe33f998
...
3. Añade un nuevo fichero Kotlin llamado `Main.kt`: clic derecho sobre el paquete dentro de `kotlin+java` > **New** > **Kotlin Class/File**.

![Menú contextual sobre el paquete de kotlin+java con la opción New > Kotlin Class/File seleccionada](img/T02/ej00-nuevo-fichero-kotlin.png)

4. Crea un repo privado en GitHub dentro de tu organización de Móvil 2026.

Puedes hacerlo con la cli de GitHub o desde la web. Lo mejor *imho* es abrir la terminal integrada de Android Studio y ejecutar lo siguiente, cambiando `Movil2026TuNombreApellidos` por el nombre de tu organización en GitHub:
...
5. Con `--remote=origin`, `gh` ya ha configurado el remote, y la autenticación tampoco debería darte problemas. Si no tienes `git config` configurado de forma global (`user.name` y `user.email`), hazlo ahora.

6. Haz un commit inicial y un push a tu repo de GitHub. Si tu rama se llama `master`, renómbrala antes con `git branch -M main`.


#### Ejercicio 1: Hola Mundo en Kotlin

1. En el fichero `Main.kt`, escribe la función `main()`, que llamará a una función `ejercicio1()`.
